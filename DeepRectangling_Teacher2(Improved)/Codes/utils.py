...
class DataLoader(object):
    ...
    def NpRandomFlip(self, img, dim):
        return np.flip(img, [dim])
    ...
    def __call__(self, batch_size):
        ...
        def data_clip_generator():
            while True:
                ...
                # ### data augment ###
                # flag0 = random.randrange(0, 2)
                # flag1 = random.randrange(0, 2)
                # flag2 = random.randrange(0, 2)
                # if flag0 == 1:
                #     input_img = self.NpRandomFlip(input_img, 0)
                #     mask_img = self.NpRandomFlip(mask_img, 0)
                #     gt_img = self.NpRandomFlip(gt_img, 0)
                #
                # if flag1 == 1:
                #     input_img = self.NpRandomFlip(input_img, 1)
                #     mask_img = self.NpRandomFlip(mask_img, 1)
                #     gt_img = self.NpRandomFlip(gt_img, 1)
                #
                # if flag2 == 1:
                #     input_img = self.NpRandomFlip(input_img, 2)
                #     mask_img = self.NpRandomFlip(mask_img, 2)
                #     gt_img = self.NpRandomFlip(gt_img, 2)
                #
                # ############
