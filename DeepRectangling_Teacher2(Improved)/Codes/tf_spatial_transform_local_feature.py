...
def transformer(U, theta, width_float = 32.,height_float = 24., name='SpatialTransformer', **kwargs):
    ...
    def _interpolate(im, x, y, out_size):
        with tf.variable_scope('_interpolate'):
            ...
            # use indices to lookup pixels in the flat image and restore 使用索引查找平面图像中的像素并恢复
            # channels dim
            ...
            # and finally calculate interpolated values 最后计算插值
    ...
    def _transform3(theta, input_dim, width_float = 32.,height_float = 24.):
        with tf.variable_scope('_transform'):
            ...
            Hs = get_Hs(theta, width_float, height_float)  # Hs保存的各个小方块对应的单应性矩阵
            ##########################################
            print("Hs")
            print(Hs.shape)
            H_array = UpSampling2D(size=(int(height_float/grid_h), int(width_float/grid_w)))(Hs)
            ...
            out_height = height
            out_width = width
            grid = _meshgrid(out_height, out_width)  # 得到标准网格点坐标
            ...
            x_s = tf.reduce_sum(tf.multiply(H_array[:,0,:], grid_row), 1)  # 和单应性矩形相乘,这是在求解变换后的坐标的过程
    ...
    with tf.variable_scope(name):
        ...
        #U = U - 1.
        warp_image = _transform3(theta, U, width_float, height_float)
